...
export abstract class AbstractCodegen<ModuleTypes, StructType> {
  ...
  // The module currently being generated by generateModule(). Generation is
  // fully synchronous and processes one module at a time, so an instance field
  // is safe. Used by generateSimpleType() to drop the module-name qualifier
  // from same-module references (see there for why).
  protected currentModule?: InternalMoveModule
  // True when the current module declares a struct/enum whose name equals the
  // module name. Such a member shadows the enclosing `namespace <module>`, so
  // same-module `<module>.<x>` references must be emitted unqualified.
  protected currentModuleSelfShadowed = false
...
`
  ...
  generateModule(
    ...
    const moduleJSName = normalizeToJSName(module.name)
    this.currentModuleSelfShadowed =
      module.structs.some((s) => normalizeToJSName(s.name) === moduleJSName) ||
      module.enums.some((e) => normalizeToJSName(e.name) === moduleJSName)
...
`
  ...
  generateSimpleType(type: string, currentAddress: string): string {
    ...
    if (parts[0] === currentAddress) {
      // Same address. References to the module currently being generated are
      // emitted inside `namespace <module>`. When that module also declares a
      // struct/enum sharing its name, the leading `<module>` qualifier resolves
      // to that member (it shadows the enclosing namespace), so every
      // `<module>.<x>` same-module reference fails with TS2694. In that case
      // drop the module segment and reference the member unqualified. Sibling
      // modules in the same file are unaffected and keep the qualifier.
      if (
        this.currentModuleSelfShadowed &&
        this.currentModule &&
        parts[1] === normalizeToJSName(this.currentModule.name)
      ) {
